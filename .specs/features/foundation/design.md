...
| ~~`auth.uid()` lê a claim da sessão por um nome que variou entre versões~~ **RESOLVIDO em T6** | `supabase/tests/database/*.test.sql` | — | Inspecionada a definição real da função no CLI 2.117.0: `auth.uid()` tenta `request.jwt.claim.sub` e cai para `request.jwt.claims ->> 'sub'`. Verificado empiricamente que as duas formas funcionam e que `anon` devolve NULL. Os testes usam `request.jwt.claims` em JSON, que é o que o PostgREST define em produção |
